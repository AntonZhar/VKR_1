...
class CheckCard:
    ...
    def check_card(self, pg: PageData):
        def back(e):
        ...
        def build_agents(agent_type):
            ...
            for agent in self.__crud.get_agents(agent_type):
                agents.append(ft.dropdown.Option(text=f'{agent[1]} {agent[2]} {agent[3]}'))
            return agents
        ...
        def set_policy_holder(e):
            ...
            for i in range(2):
                if i == 0:
                    self.__fields[7].value = self.__crud.get_id_agent_by_fio(self.__fields[6].value, True)
                else:
        ...
        dlg_modal = ft.AlertDialog(
            ...
            content=ft.Text('Полис успешно просмотрен'),
            actions=[
        ...
        def add_policy(e):
            ...
            self.__crud.change_policy_db(full_data, pg.page.session.get('creds'), pg.page.session.get('policy_id'))
            ...
            try:
                for index, driver in enumerate(self.__drivers):
                    if not self.__fields[23].value:
                        output.append([matrix[get_index_by_year()][get_index_by_experience()], bonus_malus()])
                        driver[3].value = matrix[get_index_by_year()][get_index_by_experience()]
                        driver[4].value = bonus_malus()
                    else:
                        driver[3].value = 1
                        driver[4].value = 1.17
                ...
                if not self.__fields[23].value:
                    ...
                    for i in output:
                        data.append(i[1])
                    bonus_malus = max(data)
                else:
                    bonus_malus = 1.17
                # сам калькулятор - подсчет нужных значений из функций выше
                self.__fields[26].value = round((4590.5 * 1.8 * calculate_hp(float(self.__fields[18].value)) * kvs * ko[self.__fields[23].value] * calculate_ks() * bonus_malus), 2)
        ...
        def add_driver(e):
            # добавление водителя
            add_date_picker(3)
            full_name1 = ft.TextField(label='ФИО', disabled=True)
            gender1 = ft.Dropdown(
                    label='Пол',
                    disabled = True,
                    options=[
            ...
            date_of_birth1 = ft.ElevatedButton(text='Дата рождения', icon='CALENDAR_MONTH',
                                               ...
                                               height=60)
            valid_number_VU1 = ft.TextField(label='Номер действ. ВУ:', disabled=True)
            ...
            passport_data1 = ft.TextField(label='Серия и номер паспорта', disabled=True)
        ...
        def build_fields():
            ...
            datas = self.__crud.get_osago_by_addendum(pg.page.session.get('policy_id'))
            policy_holder = self.__crud.get_agent_info_full(datas[3])
            car = self.__crud.get_car_by_id_full(datas[2])
            try:
                agent_fee = str((float(datas[15]) / 100) * 10)
            except:
                agent_fee = None
            add_date_picker(4)
            for index, i in enumerate(self.__date_picker[:-1]):
                i.value = datas[5+index]
            self.__date_picker[3].value = policy_holder[3]
            for index, driver in enumerate(json.loads(datas[12])):
                driverr = self.__crud.get_driver_by_id_all(driver)
                add_driver(None)
                self.__drivers[index][0].value = ' '.join(driverr[:3])
                for indexx, i in enumerate(self.__drivers[index][1:-3]):
                    i.value = driverr[indexx+3]
                for i in range(3):
                    self.__date_picker[4 + index*3 + i].value = driverr[8 + i]
            # создаем кнопки/поля
            self.__fields = [
                ...
                ft.FilledButton(text='Сохранить', icon='SAVE',
                                style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=5),
                                                     color=ft.colors.PURPLE), on_click=add_policy, disabled=True),
                ft.TextField(label='Номер полиса', disabled=True,
                             value=datas[0]),
                ft.TextField(label='Уч. номер аддендума', disabled=True,
                             value=pg.page.session.get('policy_id')),
                ft.TextField(label='Состояние', disabled=True, value=datas[4]),
                ft.Dropdown(
                    label='Продукт',
                    value=datas[1],
                    disabled = True,
                ...
                ft.Dropdown(
                    label='Агент (ФИО)',
                    value=self.__crud.get_agent_by_id(datas[8]),
                    disabled=True,
                    options=build_agents(True),
                    on_change=set_policy_holder
                ),
                ft.TextField(label='Код агента', disabled=True, value=datas[8]),
                ft.TextField(label='Страхователь (ФИО)', on_change=set_policy_holder, value=self.__crud.get_agent_by_id(datas[3]), disabled=True),
                ft.TextField(label='Код страхователя', disabled=True, value=datas[3]),
                ft.TextField(label='Номер телефона страхователя', value=policy_holder[4], disabled=True),
                ft.TextField(label='Марка', value=car[0], disabled=True),
                ft.TextField(label='Модель', value=car[1], disabled=True),
                ft.TextField(label='Год выпуска', value=car[2], disabled=True),
                ft.TextField(label='Пробег, км', value=car[3], disabled=True),
                ft.TextField(label='VIN-номер', value=car[4], disabled=True),
                ft.TextField(label='Гос. номер', value=car[5], disabled=True),
                ft.TextField(label='Собственник', value=car[6], disabled=True),
                ft.TextField(label='Мощность л/с', value=car[7], disabled=True),
                ft.TextField(label='СТС', value=car[8], disabled=True),
                ft.Dropdown(label='Условие владение', options=[ft.dropdown.Option('Собственность')], value=car[9], disabled=True),
                ft.Dropdown(label='Цель использования', options=[ft.dropdown.Option('Личное')], value=car[10], disabled=True),
                ft.FilledButton(text='Добавить водителя', on_click=add_driver, disabled=True),
                ft.Checkbox(label='Неограниченная страховка', on_change=block_fields, disabled=True),
                ft.FilledButton(text='Рассчитать', on_click=calculator, disabled=True),
                ft.TextField(label='Комиссия агента, %', value=datas[14], disabled=True),
                ft.TextField(label='Стоимость полиса, руб', disabled=True, value=datas[15]),
                ft.TextField(label='Комиссия агента, руб', disabled=True, value=agent_fee),
                ft.TextField(label='Страховая премия', disabled=True, value=datas[9]),
                ...
                ft.TextField(label='Комментарий эксперта', multiline=True, min_lines=1, max_lines=150, value=datas[13], disabled=True)
            ]

        pg.page.title = 'Посмотреть карточку полиса' # название страницы
