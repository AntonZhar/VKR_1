...
class Prolongation:
    ...
    def prolongation(self, pg: PageData):
        def back(e):
        ...
        # инициализация диалогового окна (оповещение о том, что полис успешно изменен)
        dlg_modal = ft.AlertDialog(
            modal=True,
            content_padding=ft.padding.all(30),
            title=ft.Text('Оповещение', text_align=ft.TextAlign.CENTER), #название диалогового окна
            clip_behavior=ft.ClipBehavior.ANTI_ALIAS,
            content=ft.Text('Полис успешно изменен'), # контент в диалоговом окне
            actions=[
                ft.ElevatedButton('Закрыть', on_click=close_dlg_off) # кнопка закрыть
            ],
        ...
        def add_policy(e):
            ...
            self.__crud.change_policy_db(full_data, pg.page.session.get('creds'), pg.page.session.get('policy_id'), True)
        ...
        def calculator(e):
            ...
            try:
                ...
                pass
        def deletedriver(e):
            #функция удаление водителя
            for index, i in enumerate(self.__drivers[int(e.control.tooltip)]):
                if index in [0, 1, 2, 5]:
                    i.value = None
                    i.disabled = True
            self.__drivers[int(e.control.tooltip)][10] = True
            pg.page.update()
        def add_driver(e):
            ...
            tooltip = str(copy.deepcopy(len(self.__drivers)))
            ...
            delete_driver = ft.FilledButton(text='Удалить водителя', on_click=deletedriver, tooltip=tooltip)
            ...
            self.__drivers.append(
                [full_name1, gender1, valid_number_VU1, KVS1, KBM1, passport_data1, date_of_birth1, graduation_date1,
                 date_receipt_first1, delete_driver, False])
            try:
        ...
        def build_fields():
            ...
            self.__fields = [
                ...
                ft.TextField(label='Комиссия агента, %', value=datas[14]),
                ...
                ft.TextField(label='Комментарий эксперта', multiline=True, min_lines=1, max_lines=150, value=datas[13])
            ]

        pg.page.title = 'Пролонгировать карточку полиса' # название страницы
